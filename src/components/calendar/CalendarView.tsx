...
const CustomEvent = ({ event }: EventProps<CalendarEvent>) => {
  return (
    <div
      className="h-full w-full flex items-center px-1 overflow-hidden"
      title={event.description}
    >
      {event.type === "Exam" && (
        <AlertTriangle size={12} className="mr-1 text-white shrink-0" />
      )}
    ...
    <div className="h-full flex flex-col space-y-4">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <span className="text-sm font-semibold text-gray-400 mr-2 uppercase tracking-wide hidden sm:inline-block">
            Layers:
          </span>

          <button
            onClick={() =>
              setViewState((s) => ({ ...s, includeUQ: !s.includeUQ }))
            }
          ...
          >
            <span
              className={`w-2 h-2 rounded-full ${
                viewState.includeUQ ? "bg-purple-600" : "bg-gray-400"
              }`}
            />
            UQ
          </button>
          ...
          <button
            onClick={() =>
              setViewState((s) => ({ ...s, includeQLD: !s.includeQLD }))
            }
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${
              viewState.includeQLD
          ...
          >
            <span
              className={`w-2 h-2 rounded-full ${
                viewState.includeQLD ? "bg-blue-500" : "bg-gray-400"
              }`}
            />
            QLD
          </button>
          ...
          <button
            onClick={() =>
              setViewState((s) => ({ ...s, includeCN: !s.includeCN }))
            }
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all flex items-center gap-2 ${
              viewState.includeCN
          ...
          >
            <span
              className={`w-2 h-2 rounded-full ${
                viewState.includeCN ? "bg-red-500" : "bg-gray-400"
              }`}
            />
            CN
          </button>
